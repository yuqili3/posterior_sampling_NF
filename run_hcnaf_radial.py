# -*- coding: utf-8 -*-
"""
Created on Sat Mar 26 16:52:00 2022
...
def train_HCNAF_cnf(cnf, dataset_name, num_samples=1000, noise_sigma=0.0, batch_size=1000):
    ...
    for i_epoch in t:
        ...
        for x, y in train_loader:
            y = y.reshape(-1, 1)
            batch_idx += 1
            loss = -compute_radial_log_p_x(cnf, torch.cat((y, x), dim=1)).mean()
...
def radial_z(batch_size, ndim_z):
    radii = torch.abs(torch.randn(batch_size, 1))
    angle = torch.randn(batch_size, ndim_z)
    angle = angle / torch.linalg.norm(angle, dim=1,keepdim=True)
    return  angle * radii
     
# Computing Loss function
def compute_radial_log_p_x(model, data):
    z, log_det_j, HyperParam = model(data)
    b, ndim_z = z.shape
    log_p_z = - (ndim_z - 1)* torch.log(torch.linalg.norm(z, dim=1)) - torch.linalg.norm(z, dim=1)/2
    ...
    with torch.no_grad():
        for i in trange(len(y_list)):
            ...
            for tl, (x, y) in enumerate(test_loader):
                ...
                z = radial_z(batch_size, ndim_z)
                x_generated, _, _  = generator(torch.cat((y_samps, z.to(device)), dim=1))
